...
import {
	...
	RoundUsersResponse,
	BetInfo,
	MyCurrentPositionResponse,
	MyGameResponse,
	PendingRewardResponse,
	StatusResponse,
	NextRound,
	LiveRound
} from "./FuzioNativePrediction.types"
export interface FuzioNativePredictionReadOnlyInterface {
	contractAddress: string
	config: () => Promise<Config>
	status: () => Promise<StatusResponse>
	myCurrentPosition: ({ address }: { address: string }) => Promise<MyCurrentPositionResponse>
	finishedRound: ({ roundId }: { roundId: Uint128 }) => Promise<FinishedRound>
	myGameList: ({
		limit,
		player,
		startAfter
	...
	myPendingReward: ({ player }: { player: Addr }) => Promise<PendingRewardResponse>
	getUsersPerRound: ({
		limit,
		roundId,
		startAfter
...
export class FuzioNativePredictionQueryClient implements FuzioNativePredictionReadOnlyInterface {
	...
	finishedRound = async ({ roundId }: { roundId: Uint128 }): Promise<FinishedRound> => {
		return this.client.queryContractSmart(this.contractAddress, {
			finished_round: {
				round_id: roundId
			}
	...
	myPendingReward = async ({ player }: { player: Addr }): Promise<PendingRewardResponse> => {
		return this.client.queryContractSmart(this.contractAddress, {
			my_pending_reward: {
				player
			}
...
export class FuzioNativePredictionClient
	extends FuzioNativePredictionQueryClient
	implements FuzioNativePredictionInterface
{
